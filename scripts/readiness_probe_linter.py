...
- References whose original text ends with ``/`` are directory-level run
  buckets, not concrete artifacts. They are PROSPECTIVE when the directory does
  not exist yet and do not affect the exit code.
...
def is_directory_bucket_reference(raw: str) -> bool:
    return raw.endswith("/")
...
def scan_doc(
    ...
    for lineno, line in enumerate(text.splitlines(), start=1):
        ...
        for match in RUNS_PATH_RE.finditer(line):
            ...
            elif is_directory_bucket_reference(raw):
